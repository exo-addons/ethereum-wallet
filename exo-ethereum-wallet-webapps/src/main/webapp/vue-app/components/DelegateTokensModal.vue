...
export default {
  ...
  methods: {
    sendTokens() {
      ...
      this.loading = true;
      try {
        this.contractDetails.contract.methods.approve(this.recipient, this.amount.toString()).estimateGas({gas: window.walletSettings.userPreferences.defaultGas, gasPrice: window.walletSettings.gasPrice})
          .then(result => {
