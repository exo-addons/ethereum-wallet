<template>
  <v-card id="walletAccountsList" flat>
    ...
    <send-ether-modal :account="walletAddress" :balance="selectedItem && selectedItem.balance" :open="sendEtherModal" no-button @sent="addSendEtherTransaction" @close="sendEtherModal = false" @error="$emit('error', $event)" />
    <!-- Contract actions -->
    <send-tokens-modal :account="walletAddress" :contract-details="selectedItem" :open="sendTokenModal" no-button @sent="addSendTokenTransaction" @close="sendTokenModal = false" @error="$emit('error', $event)" />
    <delegate-tokens-modal :contract="selectedItem && selectedItem.contract" :contract-details="selectedItem" :open="delegateTokenModal" no-button @sent="addSendTokenTransaction" @close="delegateTokenModal = false" @error="$emit('error', $event)" />
    <send-delegated-tokens-modal :wallet-address="walletAddress" :contract="selectedItem && selectedItem.contract" :contract-details="selectedItem" :open="sendDelegatedTokenModal" no-button @sent="addSendTokenTransaction" @close="sendDelegatedTokenModal = false" @error="$emit('error', $event)" />
      ...
      <v-layout row wrap>
        <template v-for="(item, index) in accountsList">
          <div v-if="(item.isContract && overviewAccounts.indexOf(item.address) > -1) || (!item.isContract && (overviewAccounts.indexOf('ether') > -1 || overviewAccounts.indexOf('fiat') > -1))" :key="index" class="accountItemContainer">
            <v-hover>
              <v-card slot-scope="{hover}" :class="`elevation-${hover ? 9 : 2}`" width="400px" max-width="100%" height="210px">
                <v-card-title dark class="primary" data-app>
                  ...
                  <v-menu v-if="!item.error" :ref="`walletAccountCard${index}`" :attach="`.walletAccountMenuItem${index}`" :class="`walletAccountMenuItem${index}`" content-class="walletAccountMenu">
                    <v-btn slot="activator" :disabled="isReadOnly" dark icon> <v-icon v-if="!isReadOnly">more_vert</v-icon> </v-btn>
                    <v-list v-if="!isReadOnly" class="pt-0 pb-0">
                      ...
                      <v-list-tile
                        ...
                        "
                      >
                        <v-list-tile-title> Send token </v-list-tile-title>
                      </v-list-tile>
                      <v-list-tile
                        ...
                        "
                      >
                        <v-list-tile-title> Delegate tokens </v-list-tile-title>
                      </v-list-tile>
                      <v-list-tile
                        v-if="enableDelegation && item.isContract && item.balance > 0 && item.etherBalance > 0"
                        @click="
                        ...
                        "
                      >
                        <v-list-tile-title> Send delegated tokens </v-list-tile-title>
                      </v-list-tile>
                      <v-list-tile v-if="!isSpace && item.isContract && !item.isDefault" @click="deleteContract(item, $event)"> <v-list-tile-title> Remove from list </v-list-tile-title> </v-list-tile>
                    </v-list>
                  </v-menu>
                </v-card-title>
                <v-card-title :class="!item.error && 'clickable'" class="accountItemContent" @click="!item.error && $emit('account-details-selected', item)">
                  ...
                  <v-spacer v-if="!item.error" />
                  <v-btn v-if="!item.error" icon class="mr-2" @click="!item.error && $emit('account-details-selected', item)"> <v-icon>fa-angle-right</v-icon> </v-btn>
                </v-card-title>
              </v-card>
            </v-hover>
          </div>
