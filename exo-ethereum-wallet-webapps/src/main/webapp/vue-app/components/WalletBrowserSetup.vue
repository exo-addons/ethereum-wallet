      ...
      <template v-else>
        <v-form ref="form">
          <v-text-field
            ...
            placeholder="Enter a password for your new wallet"
            counter
...
export default {
  ...
  methods: {
    ...
    createWallet() {
      ...
      const thiss = this;
      window.setTimeout(() => {
        const entropy = thiss.walletPassword + Math.random();
        ...
        if (!walletAddress) {
          ...
          thiss.$emit('error', 'Error saving creating new Wallet');
          return;
        }
        saveBrowserWalletInstance(wallet[0], thiss.walletPassword, thiss.isSpace)
          .then(() => {
            thiss.$emit('configured');
            thiss.loadingWalletBrowser = false;
          })
          .catch((e) => {
            console.debug('saveBrowserWalletInstance method - error', e);
            thiss.$emit('error', 'Error saving new Wallet address');
            thiss.loadingWalletBrowser = false;
          });
      }, 200);
    },
