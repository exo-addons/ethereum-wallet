...
export default {
  ...
  methods: {
      ...
      if (step > 0 && !this.gasByStep[step]) {
        if (step < 4) {
          ...
          return this.contractInstancesByStep[3].methods
            .initialize(convertTokenAmountToSend(1000000, 18).toString(), 'Token name', 18, 'T')
            .estimateGas({
      ...
      try {
        if (step < 4) {
          ...
          this.contractInstancesByStep[3].methods
            .initialize(convertTokenAmountToSend(this.newTokenInitialCoins, this.newTokenDecimals).toString(), this.newTokenName, this.newTokenDecimals, this.newTokenSymbol)
            .send({
              from: this.contractInstancesByStep[3].options.from,
              gasPrice: this.gasPrice,
              gas: gasLimit,
    ...
    finishInstallation() {
      ...
      return retrieveContractDetails(this.account, contractDetails, true).then(() => {
        this.$set(this.processingStep, this.step, true);
