                  ...
                  <div v-if="displayWalletResetOption">
                    <h4>
                      ...
                      <information-bubble v-if="!backedUp">
                        ...
                        <template slot="content">
                          Your wallet is not backed up yet. You may loose access to your funds permanently.
                          You can choose to backup your private key, encrypted by a password, on server.
                          By doing this, you will be able to download it from any device and browser.
                        </template>
                      </information-bubble>
                    </h4>
                    <template v-if="hasKeyOnServerSide">
                      <confirm-dialog
                        ref="deleteBackupOnServer"
                        message="Would you like to delete backup from server side"
                        title="Remove server backup"
                        ok-label="Delete"
                        cancel-label="Cancel"
                        @ok="removeServerSideBackup" />
                      <a href="javascript:void(0);" @click="$refs.deleteBackupOnServer.open()">
                        Remove server backup
                      </a>
                    </template>
                    <wallet-backup-server-side-modal
                      v-else
                      ref="walletBackupServerSideModal"
                      :wallet-address="walletAddress"
                      @saved="refreshFromSettings()" />
                  </div>
                </v-card-text>
              </v-card>
            </v-tab-item>
...
import WalletBackupServerSideModal from './WalletBackupServerSideModal.vue';
