...
export function sendPrivateKey(walletAddress, password, newPassword) {
  walletAddress = walletAddress.toLowerCase();

  const unlocked = unlockBrowerWallet(password && hashCode(password), null, walletAddress);
  if (unlocked) {
    try {
      saveBrowerWallet(newPassword);
    } catch (e) {
      console.debug('saveBrowerWallet method error', e);
  ...
  return fetch(`/portal/rest/wallet/api/account/savePrivateKey?address=${walletAddress}`, {
    ...
    if(resp && resp.ok) {
      ...
      localStorage.removeItem(`exo-wallet-${walletAddress}-userp`);
      localStorage.removeItem(`exo-wallet-${walletAddress}-userp-autoGenerated`);
      return true;
    }
  });
}
...
export function getPrivateKey(walletAddress) {
...
export function initSettings(isSpace, spaceGroup) {
  ...
  return fetch(`/portal/rest/wallet/api/global-settings?networkId=0&spaceId=${spaceId}`, {credentials: 'include'})
    ...
    .then(() => {
      if (window.walletSettings.userPreferences && window.walletSettings.userPreferences.hasKeyOnServerSide && window.walletSettings.userPreferences.walletAddress && !window.walletSettings.userPreferences.useMetamask) {
        return getPrivateKey(window.walletSettings.userPreferences.walletAddress).then((privateKey) => {
          if (!privateKey) {
            return;
          }
          localStorage.setItem(window.walletSettings.userPreferences.walletAddress, privateKey);
        })
        .catch(e =>  {
