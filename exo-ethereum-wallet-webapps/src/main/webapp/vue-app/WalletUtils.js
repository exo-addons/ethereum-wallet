...
export function unlockWallet(password, hash, save) {
  let unlocked = window.localWeb3.eth.accounts.wallet.length > 0 && window.localWeb3.eth.accounts.wallet[window.walletSettings.userPreferences.walletAddress];
  ...
  if (!unlocked) {
    ...
    unlocked = window.localWeb3.eth.accounts.wallet.length > 0 && window.localWeb3.eth.accounts.wallet[window.walletSettings.userPreferences.walletAddress];
  }
  if (unlocked) {
    if (save) {
...
function createLocalWeb3Instance(isSpace, useMetamask) {
  if (window.walletSettings.userPreferences.walletAddress) {
    ...
    if (useMetamask
      ...
      if (window.walletSettings.browserWallet) {
