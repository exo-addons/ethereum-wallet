...
contract('ERTToken', function(accounts) {
  it("should put 10000 ERTToken in the first account", function() {
  ...
  it("should send 10 ERTToken to the second account", function() {
	  let contractInstance;
	  return ERTToken.deployed().then(function(instance) {
		  contractInstance = instance;
		  return contractInstance.transfer(accounts[1], 10, {from: accounts[0]});
	  }).then(function(result) {
		  assert.equal(result.receipt.status, true, "Transaction failed");
	      return contractInstance.balanceOf.call(accounts[0]);
	  }).then(function(balance) {
		  assert.equal(balance, 100000 - 10, "Token balance of sender is wrong");
		  return contractInstance.balanceOf.call(accounts[1]);
	  }).then(function(balance) {
		  assert.equal(balance, 10, "Token balance of receiver is wrong");
	  });
  });
