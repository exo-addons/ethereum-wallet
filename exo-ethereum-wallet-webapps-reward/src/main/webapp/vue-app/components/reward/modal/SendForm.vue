...
<script>
import {saveRewardTransactions, saveRewardTransaction} from '../../../js/RewardServices.js';
  ...
  methods: {
    ...
    sendTokens(recipientWallet, rewardTransactions) {
      ...
      try {
        ...
        const amountToSend = this.walletUtils.convertTokenAmountToSend(amountToSendForReceiver, this.contractDetails.decimals).toString();
        ...
        return this.contractDetails.contract.methods
          ...
          .then((estimatedGas) => {
            ...
            return this.tokenUtils.sendContractTransaction(this.useMetamask, this.networkId, {
             ...
             (hash) => {
                ...
                this.transactionUtils.saveTransactionDetails(pendingTransaction, contractDetails);
              ...
              (error, receipt) => {
                ...
                errorAppended = true;
                this.appendError(`Error sending tokens: ${this.walletUtils.truncateError(error)}`);
                this.$emit('error', `Error sending tokens: ${this.walletUtils.truncateError(error)}`);
                if (!this.errors && this.transactionsSent === this.recipientsHavingAddress.length) {
                  this.$emit('close');
                }
              });
          ...
          .catch((e) => {
            ...
            if (!errorAppended) {
              this.appendError(`Error sending tokens: ${this.walletUtils.truncateError(e)}`);
            }
          })
          .finally(() => {
            this.loadingCount--;
